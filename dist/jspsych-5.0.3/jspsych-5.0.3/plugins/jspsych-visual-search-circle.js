...
jsPsych.plugins["visual-search-circle"] = function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    for (var i = 0; i < possible_display_locs; i++) {
      display_locs.push([Math.floor(paper_size / 2 + cosd(random_offset + i * (360 / possible_display_locs)) * radi - hstimw), Math.floor(paper_size / 2 - sind(random_offset + i * (360 / possible_display_locs)) * radi - hstimh)]);
    }
    ...
    function show_search_array() {
      ...
      var after_response = function (info) {
        ...
        if (info.key == trial.target_present_key && trial.target_present || info.key == trial.target_absent_key && !trial.target_present) {
          correct = 1;
        }
